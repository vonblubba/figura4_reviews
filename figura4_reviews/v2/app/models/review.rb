class Review < ActiveRecord::Base
  ...
  belongs_to :user
  acts_as_taggable
  validates_presence_of :review, :vote, :original_title, :author_id, :published, :user_id
  ...
  def to_param
    "#{id}-#{self.full_title.downcase.gsub(/[^[:alnum:]]/,'-')}".gsub(/-{2,}/,'-')
  end
  ...
  def formal_italian_title
    if self.italian_article.nil? or self.italian_article.blank?
      if self.italian_title.blank? or self.italian_title.nil?
        nil
      else
        self.italian_title
      end
  ...
  def formal_title
    if self.formal_italian_title.nil? or self.formal_italian_title.blank?
  ...
  def subtitle
    if self.italian_subtitle.nil? or self.italian_subtitle.blank?
      if self.original_subtitle.nil? or self.original_subtitle.blank?
        nil
      else
  ...
  def page_title
    "'" + self.full_title + "'" + ' by ' + self.author.full_name
  end
  ...
  def meta_description  
    "recensione del #{self.media} #{self.full_title} di #{self.author.full_name}, con note biografiche sull'autore e link sul tema"
  end
  
  def meta_keywords
    "recensione, #{self.media}, #{self.full_title}, #{self.author.second_name}, #{self.author.first_name}, #{self.tag_list}"
  end
end
